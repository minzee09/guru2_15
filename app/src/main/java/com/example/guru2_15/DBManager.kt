...
class DBManager(
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        ...
        db!!.execSQL("CREATE TABLE userInfo (UserEmail text, UID text,  UID text primary key )")
        db!!.execSQL("CREATE TABLE schedule ( Sname text, Scolor text, Sdate text, SShour text, SSminute text, " +
                "SEhour text, SEminute text, Splace text, Smemo text,FOREIGN KEY(UID) REFERENCES userInfo(UID));")
